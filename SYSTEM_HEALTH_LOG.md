...
## 2026-08-24 — Cron integrity audit (`cursor/system-integrity-audit-ee95`)
...
### Security — fixed (8th occurrence of same regression)
...
- **Lint:** 157 Prettier formatting issues auto-fixed with `eslint --fix`; 1 remaining warning (`react-refresh/only-export-components` in `question-edit-modal.tsx` — pre-existing)
- **npm ci:** `package-lock.json` refreshed — `npm ci` now succeeds (was failing on missing `lru-cache@11.5.2`)
- **npm audit:** 9 vulnerabilities (5 moderate, 4 high) in transitive deps (`undici` via nitro, `brace-expansion`, `js-yaml`, `nanoid`) — do **not** run `audit fix --force` (would break nitro)
...
### Follow-up
- **Merge staff guards to `main`** — this regression has recurred on eight cron runs because fixes land on health branches only.
