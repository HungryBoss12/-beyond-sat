...
- **Status:** **404 on every route** — `/`, `/signin`, `/signup`, `/dashboard`, `/practice`, `/banned`, `/favicon.svg`, `/auth/callback`, `/onboarding`, `/beyond-ai`, `/admin`, `/reset-password`, `/api/health`
...
- **Regression:** Site was HTTP 200 on 2026-08-26; still fully unreachable on 2026-08-27 and 2026-08-28. **Requires manual `npm run deploy` / Cloudflare dashboard check** — cron cannot fix deployment from CI alone.

### Security — fixed (12th occurrence of same regression)
...
2. **Merge staff guards to `main`** — this regression has recurred on twelve cron runs because fixes land on health branches only.
