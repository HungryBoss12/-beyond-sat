/**
 * Map embedded DOCX images to question numbers (Module 1/2).
 * Run: node scripts/map-v2-images.mjs
 */
import fs from "fs";

const xml = fs.readFileSync("tmp-math-v2/word/document.xml", "utf8");
const rels = fs.readFileSync("tmp-math-v2/word/_rels/document.xml.rels", "utf8");
const rmap = {};
...
for (const p of paras) {
  ...
  if (imgs.length) {
    hits.push({ mod, q, text: text.slice(0, 80), imgs });
  }
}
