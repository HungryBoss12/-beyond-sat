...
- **404:** `/reset-password` on deployed build (route added in this run — needs deploy)
...
### Security — fixed (14th occurrence of same regression)
...
2. **Merge staff guards to `main`** — this regression has recurred on fourteen cron runs because fixes land on health branches only.
